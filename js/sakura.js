//樱花漂浮特效 本js文件参考来源：https://www.cnblogs.com/quaint/p/12291936.html
let stop;
let staticx = true;
let img = new Image();
// 樱花数量 (添加)
let sakuraNum = 21;
let sakuraMaxs = 0.8   //樱花大小之 Max
let sakuraMins = 0.4
// 樱花循环次数, -1的时候樱花生成会无限循环 。
let limitTimes = -1;
//樱花数据
...
// 定义限制数组
let limitArray = new Array(sakuraNum);
...
function getRandom(option) {
	...
	switch(option) {
		...
		case 'y':
			...
			break;
		case 's':
			ret = Math.random()*(sakuraMaxs-sakuraMins) + sakuraMins;
			break;
		case 'r':
			ret = Math.random() * 6;
			break;
		case 'fnx':
			...
			random = -0.7 + Math.random() * (-1-(-0.7)); 
			ret = function(x, y) {
				return x + random;
			};
			break;
		case 'fny':
			random = 0.6 + Math.random() * (1.3 - 0.6)   //2.2max -0.7min
			ret = function(x, y) {
				return y + random;
			};
			break;
...
img.onload = function () {
  sakuraReady = true;
  console.log("sakuraReady成功"); // 检查这句是否出现在控制台
};
...
// 按钮点击切换开关
document.getElementById('sakura-toggle-btn').addEventListener('click', function() {
